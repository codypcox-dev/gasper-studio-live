import { HASH_FLOAT_DECIMALS } from "./types.js";
...
/** Normalize numbers for stable hashing (fixed decimal places). */
export function normalizeHashNumber(n: number, decimals = HASH_FLOAT_DECIMALS): number {
  if (!Number.isFinite(n)) {
    throw new Error(`non-finite number in hash basis: ${n}`);
  }
  const f = 10 ** decimals;
...
/** Deep-canonicalize JSON data with sorted keys and quantized numbers. */
export function canonicalizeForHash(value: unknown): unknown {
  if (value === null || typeof value === "boolean" || typeof value === "string") {
    return value;
  }
  if (typeof value === "number") return normalizeHashNumber(value);
  ...
  if (typeof value === "object") {
    ...
    for (const key of Object.keys(object).sort()) {
      ...
      if (entry !== undefined) output[key] = canonicalizeForHash(entry);
    }
    return output;
  }
...
/** Synchronous, browser-safe SHA-256 over UTF-8 text or bytes. */
...
export function hashJsonArtifact(value: unknown): string {
  return `sha256:${sha256OfCanonical(value)}`;
}
